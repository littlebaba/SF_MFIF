...
class MfDataset(BaseDataset):
    ...
    def __init__(self,opt):
        ...
        if str2_bool(opt('isTrain')):
            temp=make_dataset(opt('dir'),is_direct_dir=False,isTrain=str2_bool(opt('isTrain')),set_dataset_max=int(opt('set_dataset_size')))
#             length=len(temp)//3
#             for i in range(length):
#                 self.image_paths.append(temp[i*3:i*3+3])
